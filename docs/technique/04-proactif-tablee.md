...
- **OFF par défaut, dégradation honnête** : aucun scope requis pour démarrer ; connecteur muet/non autorisé → la ronde tourne sur le local, échec loggé. **Transitoire ≠ durable (T7 conv 12)** : la panne passagère du collecteur reste silencieuse-loggée (cahier, inchangé) ; une **expiration/révocation durable du jeton Google alerte le registre du gardien** (montre des jetons, doc `05` §2.4) — jamais une collecte agenda/mail morte en silence pendant des semaines.
...
- **Entrées** : les initiatives à annoncer (§3.2) · l'état d'écoute et les micro-creux (Smart Turn, doc `01`) · le mode du gouverneur (INTERACTIF/REPOS + drapeau dev/focus — *posé aussi par le calque JEU, doc `05` §2.2 ; AT5 conv 13*) · l'état du toggle voix et de l'interrupteur proactif.
...
- **Chemin des rappels — non gated par l'interrupteur** : un **contrôle d'échéance déterministe** (le gouverneur teste `tasks.due_at`, **0 quota** — aucune décision LLM, Yohann l'a déjà demandé) fait **sonner** un rappel quand c'est l'heure, **même proactif OFF — et même en JEU** (décisions Yohann, conv 11 + conv 13/AT5 : *« elle a remarqué un truc »* se coupe ; *« elle tient ce que tu lui as demandé »* non — le garde-fou temporel protège ton attention contre *ses* initiatives, jamais contre *tes* engagements datés). Le rappel reste soumis à la **priorité interactive** (micro-creux) et au **toggle voix** (systray si silencieux).
...
- **Le ducking en tablée (AT10, conv 13)** : armé par **sa voix et son nom seulement** — la musique baisse quand *elle* parle (`evt.tts.start` → duck, remonte après) et au wake ; **le VAD ambiant ne duck jamais** (la soirée appartient aux convives — et l'AEC loopback rend le duck inutile à la compréhension, doc `01` F2/M4). Complète le périmètre « armé par l'état » de F3 (doc `01` §4.6).
...
3. **Après distillation, le tampon est purgé** — le verbatim brut des tiers ne survit jamais. En report/SECOURS, le tampon **persiste jusqu'à la nuit** (backlog borné), jamais perdu → **« la nuit ne dépend jamais du micro » tenu** (la source, c'est le tampon). **Le jour sacrifié (AT3, conv 13 — l'intersection avec T23 doc `02`, tranchée)** : si le jour de la soirée est **sacrifié** (au-delà du backlog borné, doc `02` §4.4), son tampon est **purgé sans distillation**, tracé dans `consolidation_runs` (« soirée non distillée ») et **dit** — jamais silencieux. La chronique de ce jour perd la soirée, assumé : **la vie privée des tiers (A31) prime sur la complétude de la chronique** — le verbatim des tiers ne survit à *aucun* scénario, pas même la panne longue.
...
- **La discrétion en miroir (AT9, conv 13)** : la vie de **Yohann** devant les tiers est protégée par la **disposition 12** (doc `03` §3.4) — le filtre est à sa **bouche**, jamais à sa mémoire (l'affleurement reste entier en tablée, doc `02` §4.5) ; **repli nommé** si l'observation l'exige : moduler l'affleurement en tablée.
...
- **Retouche actée conv 13 (audit transversal solo — AT3, validée par Yohann)** : le **jour sacrifié** (T23 doc `02`) tranché pour la tablée — tampon **purgé sans distillation**, tracé (`consolidation_runs` « soirée non distillée ») + **dit** ; la soirée perdue est assumée (**A31 prime sur la complétude de la chronique**) — §4.10 · §5 ; ligne d'honnêteté portée au doc `02` (T23 + critère 11).
- **Retouche actée conv 13 (audit transversal solo — AT4, validée par Yohann)** : la précision **T7 de l'audit croisé conv 12** (tracée au doc `05` §7 mais jamais portée ici) est **appliquée avec retard** — §2.2 : panne transitoire du collecteur silencieuse-loggée (inchangé) ; **expiration/révocation durable du jeton Google → alerte au registre du gardien** (montre des jetons, doc `05` §2.4).
