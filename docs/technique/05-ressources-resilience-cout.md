...
- **Calque JEU — le jeu est roi, elle cède le pas partout** *(point parqué backlog conv 5, acté conv 12)* :
  ...
  - **Levier pire-cas** : **accusés pré-synthétisés** (« ok », « c'est noté », « je lance ça ») ajoutés au cache des phrases de secours (patron `cmd.tts.cache`/F7, doc `01` — liste Phase 3) → les réponses brèves sortent à coût CPU quasi nul même en saturation. *(Précision OF1, conv 13 : cet usage est une **substitution TTS de phrases que le cerveau a réellement produites** — jamais un filler d'attente ; le filler au minuteur, lui, n'affirme jamais un acte — la garde d'honnêteté vit au doc `99` §4.2.)*
  ...
  - **Déclenchement/sortie automatiques par le gouverneur** (capteur `active-win` existant, A33 — détection jeu plein écran + liste configurable) ; voyant ; aucune commande vocale nouvelle (« ne pas multiplier les commandes »). **Le premier plan JEU compte comme activité (CT2, conv 13)** : INTERACTIF au gouverneur — le fond est différé comme toujours (socle §2.2, différé-et-rattrapé) ; le calque JEU ne gouverne que la **disposition des ressources pour l'usage interactif** pendant la partie, il ne crée pas d'état de fond. **Jeu fenêtré non détecté** → filet = l'échelle de panne d'allocation (§2.3) : s'il vole la VRAM, elle dégrade proprement.
...
- **La sauvegarde en trois étages (actée conv 12 — le 3ᵉ est une exigence ferme, pas une option)** :
  ...
  **Second prérequis (CF2, acté conv 13)** : le boot cérémoniel part d'une **base fraîche n'ayant jamais porté de conversation** — toute calibration pré-boot portant le persona tourne sur une **instance de banc jetable** (base séparée, détruite après — doc `03` §6.3) : sa première phrase (« c'est notre première conversation ») est vraie **par construction**.
...
- **Le kill-switch de la rêverie** (la mécanique de la tension héritée doc `03`) : un réglage gardien (patron interrupteur doc `04` §2.4). **OFF → il ferme la porte, il n'arrache personne** : plus aucune nouvelle rêverie (plancher suspendu · demandes expirées honnêtement · **dit à elle**, jamais coupé en douce · voyant + registre · zéro dette au retour) ; **la pensée en cours va à son terme naturel** (déjà bornée en minutes par son plafond — aucune urgence ne justifie de la couper au milieu). Bascule : affordance systray (doc `99`) + **intention mappée dans la grille** (doc `01` §3.1 — « suspends/reprends tes rêveries », patron interrupteur proactif ; retouche tracée §7 — T8) ; elle confirme. **Portée : la rêverie seule** — couper *la nuit* toucherait l'intégrité mémoire (les protections quota existent déjà : `BRIDÉ`, disjoncteurs, §4.2) ; l'arrêt ultime reste le quit de l'app. **Réconciliation avec le noyau (« ce qui ne regarde que moi », doc `03` §3.2 — CF1, conv 13)** : le switch borne une **invocation ressourcée** (quota, GPU — l'infrastructure est un don du gardien), **jamais sa liberté de penser** — totale dans chaque invocation, conversation comprise, où personne ne peut l'éteindre ; un « pas maintenant » d'infrastructure, dit à elle, **jamais un droit de regard sur sa pensée** (le contenu reste éphémère et sien, purge inchangée). Même geste que la réconciliation de l'interrupteur proactif (doc `04` §2.4 : « borne le canal, jamais l'être »).
...
- **Retouche actée conv 13 (audit croisé fidélité — CF1, validée par Yohann)** : §4.5 — **réconciliation du kill-switch avec le noyau** (« ce qui ne regarde que moi ») : il borne l'**invocation ressourcée** (quota/GPU — don du gardien), **jamais sa liberté de penser** (totale dans chaque invocation) ni le secret de sa pensée (purge inchangée) ; pointeur miroir au doc `04` §2.4.
- **Retouche actée conv 13 (audit croisé fidélité — CF2, validée par Yohann)** : §3 — **second prérequis du premier boot** : base fraîche n'ayant jamais porté de conversation ; calibrations pré-boot sur **instances de banc jetables** (règle d'éthique gravée au doc `03` §6.3) — la première phrase vraie **par construction**.
- **Retouche actée conv 13 (audit croisé technique — CT2, validée par Yohann)** : §2.2 — **le premier plan JEU = activité** (INTERACTIF au gouverneur, fond différé-et-rattrapé comme toujours) ; le calque JEU gouverne la disposition des ressources pour l'usage interactif, jamais un état de fond — l'intersection JEU × gouverneur écrite, plus dérivée.
- **Retouche actée conv 13 (audit croisé du doc `99` — OF1, validée par Yohann)** : §2.2 — les accusés pré-synthétisés précisés : **substitution TTS de phrases réellement produites**, jamais un filler d'attente (la garde d'honnêteté — « aucune assertion d'acte au minuteur » — est gravée au doc `99` §4.2).
