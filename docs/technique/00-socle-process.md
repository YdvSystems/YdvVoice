...
- **Rôles** : le **sidecar Python héberge** le serveur (WebSocket + REST) sur le **port** que l'orchestrateur lui passe au spawn (§4.3) ; l'**orchestrateur Node est le client**. *(Cohérent avec l'idiome Plume : le process spawné tient le port.)*
