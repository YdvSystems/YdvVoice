...
| `memory_store` | propose un **fait** (contenu, catégorie, importance), une **empreinte prioritaire**, une **tâche** (`due_at` — doc `04` §3.1, retouche conv 11), ou une **note à elle-même** (`self_notes`, en rêverie — doc `03` §2.1/T9) | écrit **uniquement dans l'ensemble {`facts` en `PROVISIONAL` · `imprints` · `tasks` · `self_notes`}, discriminé par le scope de l'invocation** (CT1 conv 13) : **conversation** → `facts`/`imprints`/`tasks` · **rêverie** → `imprints`/`self_notes` seuls (`facts`/`tasks` refusés — doc `03` T10) — **aucun chemin vers les tables identitaires** (§5) ; plafond d'appels par session ; hors scope → refus normalisé |
...
| `turn_signals` | `conversation_id` · `reason` (fin de tour) · barge-in (survenu, position) · affect (valence · énergie · confiance — **nullable, OFF par défaut, verrou Yohann**, doc `01` §2.4) · **valeurs du tag d'humeur** (deltas proposés par curseur · drapeau d'agacement de valeur — émises par elle en queue de tour, doc `03` §2.2 ; AT6 conv 13) · `captured_at` — table séparée : OFF-safe, **rétention bornée** (exception nommée §5, valeur rubrique 7) |
...
- **Au fil de l'eau — l'affleurement** (la couche 2 d'A13 mécanisée) : à chaque tour **en conversation** (jamais VEILLE/DICTÉE — **actif en tablée** : la discrétion sur ce qui affleure est une **disposition**, doc `03` §3.4 disposition 12/AT9 ; repli nommé : le moduler si le vécu l'exige), l'orchestrateur embedde le transcript courant et cherche, borné ; **seuls les souvenirs au-dessus du seuil** sont glissés au tour suivant (« ça affleure ») — rare et signifiant. Le pull (`memory_search`) reste son acte délibéré ; l'affleurement est son association spontanée.
...
2. Migration **corpus par corpus, par priorité** (`facts` d'abord, puis `chronicle` / `sessions` / **`introspection`** *(doc `03` §2.3 — AT7 conv 13)* / `knowledge` — les corpus *vécus* avant le corpus *lu*) : reconstruction en **table ombre** (tâche de fond gouvernée, resumable — unités+curseur) → **bascule atomique** → le corpus retrouve sa jambe sémantique. *(`vec_initiatives`, lui, est **flushé, pas migré** — doc `04` §3.2.)*
...
11. **Rattrapage multi-jours** : PC éteint 3 jours → 3 entrées de chronique datées de **leurs** jours, marquées écrites plus tard, traitées du plus ancien au plus récent ; au-delà du backlog borné → **jours sacrifiés tracés**, brut cherchable (T23) — **sauf le tampon de tablée d'un jour sacrifié : purgé sans distillation, tracé + dit (AT3, doc `04` §4.10)**.
...
- **Retouche actée conv 13 (audit transversal solo — AT3, validée par Yohann)** : **exception d'honnêteté au T23** — un jour **tablée** sacrifié ne laisse pas le brut des tiers cherchable : son tampon (`tablee_buffer`, doc `04`) est **purgé sans distillation, tracé + dit** ; A31 prime sur la complétude de la chronique (§4.4 · critère 11 ; politique gravée au doc `04` §4.10).
- **Retouche actée conv 13 (audit transversal solo — AT6, validée par Yohann)** : §3.1 — la déclaration de `turn_signals` gagne les **valeurs du tag d'humeur** (deltas · drapeau de valeur), écrites là par le doc `03` §2.2/T2 depuis conv 10 mais jamais déclarées au schéma (patron affect : colonne déclarée au `02`, producteur pointé).
- **Retouche actée conv 13 (audit transversal solo — AT7, validée par Yohann)** : §4.8 — la file de migration d'espace d'embedding gagne le corpus **`introspection`** (enregistré par le doc `03` §2.3 *après* l'écriture de la liste, conv 10) — placé avant `knowledge` (les corpus vécus avant le corpus lu) ; rappel `vec_initiatives` = flushé (doc `04`).
- **Retouche actée conv 13 (audit transversal solo — AT9, validée par Yohann)** : §4.5 — l'affleurement est dit **actif en tablée** ; la discrétion sur ce qui affleure = **disposition 12** (doc `03` §3.4) — le filtre à sa bouche, jamais à sa mémoire ; repli nommé (moduler l'affleurement en tablée) si l'observation l'exige.
- **Trace consolidée (conv 11, portée avec retard conv 13/CT1)** : §2.3 — `memory_store` a gagné la cible **`task`** (retouche actée conv 11, tracée au doc `04` §7 et inline au contrat ; la ligne consolidée manquait ici).
- **Retouche actée conv 13 (audit croisé technique — CT1, validée par Yohann)** : §2.3 + critère 14 — le contrat de `memory_store` **réaligné en une seule vérité par scope** : ensemble {`facts` PROVISIONAL · `imprints` · `tasks` · `self_notes`} (conversation : facts/imprints/tasks · rêverie : imprints/self_notes) ; le critère 14 n'exigeait encore que faits/empreintes (jamais mis à jour après les extensions convs 10-11) et **contredisait le contrat qu'il vérifie**.
