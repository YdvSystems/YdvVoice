...
| `memory_store` | propose un **fait** (contenu, catégorie, importance), une **empreinte prioritaire**, ou une **tâche** (`due_at` — doc `04` §3.1, retouche conv 11) | écrit **uniquement** `facts` en `PROVISIONAL`, `imprints`, ou `tasks`, **selon le scope de l'invocation** (conversation : les trois ; rêverie : `imprints`/`self_notes` seuls, `facts`/`tasks` refusés — doc `03` T10) — **aucun chemin vers les tables identitaires** (§5) ; plafond d'appels par session ; hors périmètre → refus normalisé |
