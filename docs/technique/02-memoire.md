...
| `memory_store` | propose un **fait** (contenu, catégorie, importance) ou une **empreinte prioritaire** | écrit **uniquement** `facts` en `PROVISIONAL` ou `imprints` — **aucun chemin vers les tables identitaires** (§5) ; plafond d'appels par session ; hors périmètre → refus normalisé |
...
3. Suppression **transactionnelle et complète (T4)** : faits + `fact_sources` + `fact_relations` + FTS + vecteurs + tours (`conversations`) + `imprints` et `turn_signals` ancrés sur ces tours + résumés concernés (+ documents si demandé). Les **sources pendantes de faits conservés** (qui pointaient vers un tour effacé) sont marquées **« source effacée »** — la provenance le dit honnêtement, la chaîne de citabilité ne casse pas en silence. **Chronique (T3)** : jamais d'UPDATE — l'entrée du jour est supprimée puis **réinsérée expurgée, marquée « expurgée le … »** (ou supprimée entière si tout le jour est concerné) ; jamais de faux journal.
4. **Le fil Claude Code est invalidé (T1)** : le verbatim survivrait dans les transcripts de session du CLI — donc rotation forcée de session, **interdiction de `--resume` du fil taché** (mise à jour de `session_state`, socle), purge du fichier de session CLI. Sans ça, un `--resume` réinjecterait le contenu effacé.
...
| Table | Colonnes (rôle) |
|---|---|
| `imprints` | `id` · `conversation_id` (ancrage au moment vécu) · `nature` · `priority` · `noted_by` (micro / outil / couche 3) · `consumed` (par la nuit) · `created_at` — **le substrat des « empreintes du jour » (A18/A19)** ; la sémantique du tri → doc `03` |
...
| 5 | **Étages couche 3** — lien · couronne · **valeurs proposées** (mécanisme (ii) A15) · **journal du devenir** (la trace datée/sourcée de ce qui a changé en elle) · remise à plat de l'humeur — **vérification d'ancre AVANT** (le gravé n'a pas bougé) ; contenu → doc `03` | couche 3 |
