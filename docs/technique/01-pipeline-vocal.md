...
- **VRAM** (avec doc `05`) : co-résidence Whisper+Kokoro possible ou alternance stricte ; temps de swap RAM→VRAM ; coût du prewarm ; coût de l'autorisation transitoire `cmd.tts.cache` au boot. **[Note conv 52 — écart produit, renvoi `plan/01` §7 (V11 BÂTI)]** dans le produit bâti, l'**alternance Whisper↔Kokoro est SANS OBJET** : le **réveil EST le STT** (le wake-model écarté conv 27 → STT résident sur le GPU **même en VEILLE**) et la **voix est Piper A20 CPU** (pas de Kokoro/GPU) → **seul modèle GPU = le STT, set GPU invariant**. V11 pose donc le contrat `cmd.model.policy` (enregistré) + la remontée `evt.model.*` (+VRAM) + le **repli CPU réactif** sur refus d'allocation ; les dynamiques **proactives** (swap JEU→CPU, éviction SECOURS, prewarm au wake) restent la **politique de réponse du doc `05`** (à bâtir) — la conception §4.5 (« résidence alternée A35 ») est conservée pour l'ère où un modèle GPU concurrent (cerveau de secours Phi-4-mini) existera.
...
- **[Note conv 57 — écart produit, renvoi `plan/01` §7 (V12 BÂTI)]** §4.6/critère 11 : la remontée « **après la réponse** » (par tour) est **supersédée par la DÉCISION A de Yohann** — duck par **ÉTAT**, le temps de la conversation ENTIÈRE (la remontée par tour produisait en conversation suivie le yo-yo que le But de F3 interdit, et l'utilisateur commençait chaque phrase par-dessus le son plein) ; la remontée se fait à la **sortie de conversation** (clôture → VEILLE · pause · garde R-1). `evt.vad.start` n'est plus un **déclencheur** en conversation (l'état couvre — il reste une entrée câblée testée no-op) ; en VEILLE il n'a jamais ducké (inchangé). PAUSE ≡ VEILLE pour le ducking (convergent §4.1). AT10 (tablée : voix + nom seuls) et S9 (dictée désarmée) inchangés ; M4 tenue. Mécanisme : sessions WASAPI par app via helper persistant piloté par l'orchestrateur — la persistance per-app de Windows (mesurée) impose un write-ahead + filet boot, détail au `plan/01` §7.
- **[Note conv 59 — écarts produit, renvoi `plan/01` §7 (V14 BÂTI)]** §2.4 (01-H) : le capteur d'affect est bâti. **Moteur retenu ≠ le défaut nommé** : `emotion2vec` écarté SUR PIÈCES (sortie CATÉGORIELLE 9 classes — le « signal doux » continu exigerait un mapping inventé, contre le principe « jamais d'étiquette » ; deps funasr/modelscope) → **wav2vec2-emotion** (l'alternative nommée au §2.3 : audEERING w2v2-L-robust-12 emotion-msp-dim, ONNX officiel, sortie NATIVE arousal/dominance/valence, zéro dep). « Une évaluation par tour (à `evt.turn.end`) » : dans le produit, `turn.end` n'existe qu'en **conversation** (V5 `_armed_at_open`) → pas d'affect sur l'ouvreur d'éveil ni en veille. Le **verrou** « n'évalue que si locuteur = Yohann » est réalisé au niveau du TOUR (les verdicts V6 sont PAR SEGMENT : dernier-verdict-par-segment fait foi ; ≥ 1 « yohann » ET 0 « inconnu » dans la fenêtre du tour, sinon muet). La **confiance** du payload = auto-cohérence MESURÉE de deux lectures (fenêtre entière vs dernière moitié — un modèle dimensionnel n'a pas de proba native ; jamais un chiffre inventé). **OFF par défaut** tenu : monté par `SOPHIA_AFFECT=1` (rien ne consomme `evt.affect` avant le doc `03`). La **ligne de base personnelle** = collectée aux sessions juge (`affect` dans l'historique) ; seuils d'émission conservateurs (calibration). Détail complet (banc, coût ~1,4 s/tour en fond, licence CC-BY-NC-SA 4.0) au `plan/01` §7 (V14 BÂTI).
- **[Note conv 60 — écarts produit, renvoi `plan/01` §7 (V15 BÂTI)]** §4.8 (S10) : la resynchronisation au respawn est bâtie — détection du pipeline périmé (port/pid témoins), démontage propre, **rebuild par le chemin unique** avec l'ordre S10 dedans. **Étape 2 (`cmd.enroll.push`) = JALON D'ORDRE HONNÊTE (écart A-b acté conv 60)** : rien n'est poussé — l'ancre est **vendorisée au sidecar** et le centroïde V6 se construit au runtime (l'enrôlement réel = doc `04`/premier boot) ; le « source de vérité = WAL orchestrateur » du §3.2 décrit l'ère de l'enrôlement, pas encore advenue ; l'ack dit l'état RÉEL de l'ancre (`absent|warming|monte|warm_failed`). Au **boot nominal** (ph. 5), l'ordre est enroll→cache avec la politique partant à PRÊT via la résidence (émetteur unique S7) — l'ordre STRICT complet vaut au respawn/rebuild, là où ce § le grave (bénin : set GPU invariant, note conv 52). « Resynchroniser la politique COURANTE » est réalisé par le **rebuild-à-neuf** (résidence NEUVE → la politique courante part toujours), pas par un resync dédié ; **l'état d'écoute est ré-exécuté (écart C-c acté)** : ÉCOUTE re-posée + phrase de retour (texte = domaine Yohann) ; PAUSE re-posée avec la pensée transférée SANS phrase ; VEILLE silencieux. « Énonciations en vol = échec terminal » tenu (settled au démontage, jamais re-dites). §2.3 : la **sélection par config au spawn** existe pour la prise `stt` (`SOPHIA_STT_ENGINE` — les autres rôles restent au défaut, extension au besoin) ; « échec du provider cloud → retour automatique au local + notification honnête » = repli au warm + `evt.model.loaded {degraded, reason}` (canal V11, zéro type nouveau) ; **l'événement de coût par appel payant = DORMANT** (aucun provider payant réel — le cost-guard naîtra avec le premier vrai provider) ; la **suite de conformité par contrat** vit en pytest (stt/turn/speaker/affect : gardes honnêtes [un moteur KO ne produit jamais un faux verdict] + payloads aux clés exactes — la prise `affect` comprise) ; `wake` (sans moteur depuis conv 27), `vad` et `tts` (couverts U-V2/U-V7) tracés sans matrice dédiée. Détail complet (témoins composés, handoff, phrase de retour, audit 8 regards, test vécu kills réels) au `plan/01` §7 (V15 BÂTI).
- **[Note conv 58 — écarts produit, renvoi `plan/01` §7 (V13 BÂTI)]** §4.7/§2.2 (B2) : « charge **Kokoro** → synthétise → décharge » est **sans objet côté VRAM** dans le produit (la bouche = Piper A20 **CPU**, conv 33-34/47) → l'autorisation transitoire est **réinterprétée** : un moteur Piper chargé **en RAM** le temps de la pré-synthèse, puis **libéré** (l'esprit « une autorisation qui se referme » tenu — même famille d'écart que la note conv 52 sur §4.5). §4.7 « le sidecar, seul, joue » : écrit au temps du **monolithe** ; dans l'archi 2 process (conv 47), le filet ENTIER vit dans les **OREILLES** (décision A conv 58 — la détection [le serveur voit ses clients] et le déclencheur [la fin du tour] n'existent que là ; elles jouent le WAV pré-rendu elles-mêmes, l'AEC annulant le loopback). **Portée honnête du « jamais de silence »** : la mort du **process** Electron emporte les sidecars (Job Object du socle, conv 35 — voulu) → le filet couvre « canal coupé ANORMALEMENT, oreilles vivantes » ; une déconnexion **PROPRE** (close frame — arrêt T6) est un départ volontaire, jamais une panne (l'épisode est consommé) ; le « voyant systray » de S11 est impossible **pendant** l'épisode (le voyant EST l'orchestrateur parti) — la phrase l'a dit une fois, le boot suivant repart propre. Le déclencheur « timeout » nommé au §7-calibration n'est pas bâti (le discriminant = la déconnexion WS) ; la « durée d'épisode » = jusqu'à la reconnexion. Détail complet (codes de close mesurés, tours fantômes, RETRO_MAX) au `plan/01` §7 (V13 BÂTI).
