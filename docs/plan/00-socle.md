...
- **Contenu** : ouvrir SQLite en **mode WAL** · `db/schema-00.sql` = les 4 tables socle `governor_watermarks` · `governor_budget_ledger` · `session_state` · `runtime_flags` (colonnes selon `00` §3) · **écrivain unique = orchestrateur** (F2 : le sidecar n'a **aucune** poignée d'écriture) · **`PRAGMA foreign_keys=ON`** à la connexion d'écriture (**retouche conv 16** : sans lui, les FK des couches `02`/`03` seraient purement documentaires — SQLite le laisse OFF par défaut ; §7) · **`session_state` gagne `current_session_id`** (**colonne dans `schema-00`** — définition socle de `session_state` — **nullable, sans FK**, donc **présente dès le socle-seul** ; **cible `sessions` déclarée en `schema-02`** ; **NULL/inutilisée tant que `02` n'existe pas** — #4 croisé conv 16 ; le socle **garde** `claude_session_id`, dont il reste propriétaire pour `--resume`, et `sessions` (02) **cesse de le dupliquer** — **M-1 réorienté conv 16**, §7) · **`session_state` gagne aussi `secours_tainted`** (**bool, défaut 0, colonne dans `schema-00`** — le marqueur « **fil non-reprenable** » posé au **1ᵉʳ tour vécu en SECOURS, atomique avec l'écriture du tour** [patron T1 « fil taché » de `02`] ; lu à toute reprise [runtime, boot, reboot en plein épisode] → session fraîche + filet A13 plutôt qu'un `--resume` troué ; **écrit par la logique SECOURS de `plan/05` R5**, **0 au temps socle-seul** ; `00` §7/M-1 l'anticipait déjà [« `05` §4.1/AT2 pose une colonne de cycle-de-vie du fil sur `session_state` »] — **couture `plan/05` conv 20**, §7) · `busy_timeout` en ceinture.
...
- **U-T1** tables + round-trip + interdit d'écriture sidecar + **`foreign_keys=ON` (violation FK refusée)** + **`session_state.current_session_id` nullable** (M-1/conv 16) + **`session_state.secours_tainted` présent, défaut 0** (couture `05` conv 20 — le comportement [marque atomique + reprise fraîche] est prouvé côté `05`/I-R5).
...
- **U-T4** snapshot ouvrable + **rotations (snapshot garder N · audit taille/âge)** + audit tronqué toléré + `quick_check` + **assertion « zéro contenu conversationnel » sur le JSONL (AF-10)** + **flux d'effacements dédié présent et EXEMPTÉ de rotation (croisé Opus conv 18 F1)** + **chaque snapshot porte un « repère de crue » (dernier `erasures.id`/horodatage de la base) — croisé ciblé conv 20 G-A**.
...
- **[Retouche socle ← gravure de `plan/05` (conv 20) — `session_state.secours_tainted`]** `plan/05` R5 (détecteur / bascule SECOURS) réalise la garantie **T3/AT2** : « un fil troué par un épisode SECOURS ne se reprend jamais, y compris à travers un reboot » — la marque « non-reprenable » est posée dans `session_state` au **1ᵉʳ tour vécu en SECOURS, atomique avec le tour** (patron T1 « fil taché » de `02`). Cette garantie exige une **colonne de cycle-de-vie du fil** sur `session_state` — que **ce plan §7 (M-1) anticipait déjà** (« `05` §4.1/AT2 pose une colonne… ») **sans la déclarer**. **Gravé** : `session_state` gagne **`secours_tainted`** (bool, défaut 0, **colonne dans `schema-00`**, T1), **0 au temps socle-seul**, **écrite par la logique SECOURS de `plan/05` R5** (le socle la déclare, `05` l'écrit — même patron que `current_session_id` déclaré pour `02`). **Aucune inversion de stratification** (le socle est bâti/testé seul, colonne inutilisée sans `05`) ; **aucune retouche de la structure T0→T8**. Renvoi `plan/05` §7 + **`technique/00` §3** (schéma `session_state`, sur Go). Mécanique.

- **[Retouche socle ← croisé ciblé de `plan/05` (conv 20) — repère de crue (T4) + fail-safe de l'alerte-à-la-restauration (T5), G-A]** le croisé ciblé a débusqué un **fail-open** dans l'alerte AF-1 : au restore-catastrophe, si le **réplica-flux hors-machine n'est pas restauré au chemin local** (objet séparé, `plan/05` couture #2), T5 lit un flux **absent** → « zéro effacement postérieur » = **pass silencieux** → résurrection silencieuse. **Corrigé, deux retouches (aucune touche à T0→T8)** : **T4** — chaque snapshot enregistre un **repère de crue** (dernier `erasures.id`/horodatage connu de la base) ; **T5** — l'alerte devient **fail-safe** : un flux **absent/tronqué/en retard sur le repère de crue** ⇒ **ALERTE**, jamais un pass silencieux ; + la **procédure de restauration-catastrophe restaure le réplica-flux à son chemin local AVANT le premier boot** (versant RESTORE de la couture #2, gravé côté `plan/05` R0). Tests U-T4/I-10 mis à jour. Renvoi `plan/05` §7 (couture #2) + **`technique/00` §Durabilité** (sur Go). Mécanique.

- **[conv 21 — dette d'effacement nommée par le croisé de `plan/99` (F-99-2), sur Go de Yohann]** le croisé du plan d'assemblage a débusqué une **complétude d'effacement manquante** touchant T8 : **T8 rote le `session_id` sur « nouvelle conversation » (`--resume` du fil courant) mais ne SUPPRIME jamais l'ancien fichier de session**. Les fichiers de session des **conversations PASSÉES** s'accumulent dans `G:\Sophia\sessions\` avec leur verbatim, **hors fouille M2 et hors cascade de l'effacement souverain** (`02` M8 ne purge que le fil **taché courant**, M8-4). Un « oublie X » ne les atteint donc pas → **l'effacement n'est pas complet au niveau des fichiers de session rotés**. **Dette Phase 3 nommée** (déjà en vigilance CLAUDE.md « purge des fichiers de session CLI (T1/T8/T13 + piste `G:\Sophia\sessions\`) ») : à la rotation **et** à l'effacement souverain, les fichiers de session de conversation concernés doivent être **purgés/bornés** (pas seulement le fil courant) ; les fils rotés étant **opaques/non-indexés** (pas de purge sélective de X possible), la branche réalisable = **rétention bornée à fenêtre DITE** (miroir de la fenêtre de sauvegarde `plan/05` R0, « dite » — re-croisé conv 21) — mécanisme Phase 3, à trancher côté T8 (rotation) + `02` M8 (effacement). **Aucune mécanique changée ici — dette documentée à sa maison** (`plan/99` O4 la nomme mais ne la ferme jamais : c'est du socle/mémoire). Renvoi `plan/02` §7 + `plan/99` O4/§6. Répercussion `technique/00` §3 (`session_state`/rotation) sur Go.

- **[AUDIT CORPUS FABLE (intermède final, 2026-07-11) — back-ref FC-2, validée par Yohann]** la dette des fichiers de session rotés (F-99-2, ci-dessus) gagne un **cas aggravé, gravé à la checklist `plan/99` §6(e)** : le fil de session d'une **invocation tablée** (fraîche, `04` Q9/C8) porte le verbatim des **tiers** — contre A31 « ne survit à aucun scénario ». **Branche propre, plus forte que la fenêtre DITE** : ce fil-là est **purgeable ENTIER à la clôture de la tablée** (patron invocation autonome — marque `pending_ops` `purge-session-file` + sweep T5 Phase 2, mécanique existante) ; rien de durable perdu. Part socle : T8 n'est pas modifié (la rotation reste la sienne) — la purge du fil tablée est un geste de `04` Q9/Q11, rejoué par le sweep d'ici. Fermeture Phase 3. *(Synthèse : `docs/journal/audits/AUDIT-fable-corpus.md`.)*
