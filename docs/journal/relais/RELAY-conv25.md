...
- **Décisions gravées (`plan/01` §7)** : **F6 = PASSÉ, repli nommé NON nécessaire** (reste spécifié, non activé) · **seuil retenu ≈ 0,25–0,30** (calibration par utilisateur) · périmètre prouvé = **à la voix, au micro** ; **far-field acoustique = passe #5** (rig micros, ère matérielle — modèle déjà entraîné sur des « Sophia » lointains).
