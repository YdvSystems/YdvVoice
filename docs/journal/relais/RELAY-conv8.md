...
- **✅ `00-socle-process.md` GRAVÉ** — décisions 00-A→00-E + audit F1–F4 :
  ...
  - **00-D** **supervision sidecar = idiome interne éprouvé** (port libre dynamique + retry TOCTOU + pidfile/anti-recyclage + readiness + escalade SIGTERM/SIGKILL + drain stdio + hygiène env) ; respawn déterministe.
...
- **Empirie (audit source de vérité)** : **un précédent Windows interne éprouvé** du process-lifecycle (process-lifecycle interne) → idiome réutilisé (00-D). Toolchain : **Node 24.13 · Python 3.14**.
...
- **Dépendance Anthropic = VIGILANCE N°1** (#4) — à documenter techniquement en `05` (health-check, 401, ladder). Quota Yohann fortement sollicité → x20.
