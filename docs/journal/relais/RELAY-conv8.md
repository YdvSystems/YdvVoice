...
- **Empirie (audit source de vérité)** : **un précédent Windows interne éprouvé** du process-lifecycle → idiome réutilisé (00-D). Toolchain : **Node 24.13 · Python 3.14**.
...
- **Dépendance Anthropic = VIGILANCE N°1** (#4) — à documenter techniquement en `05` (health-check, 401, ladder). Quota x5 déjà fortement sollicité par l'usage pro → x20.
