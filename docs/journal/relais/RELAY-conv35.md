...
2. **AUDIT DU SOCLE : 2 tours, tous findings corrigés DANS la conv (jamais déférés en facilité).** Croisé (fidélité+robustesse) → **2 MAJEUR** (M1 spawn sans handler `error` ; M2 anti-recyclage PID trop grossier) + ~14 MINEUR. **RE-CROISÉ des corrections** → **1 MAJEUR dans MA propre correction** (rotation par âge piégée par le **tunneling NTFS** = perte de journal) + ~10 MINEUR. **Tout corrigé + testé.** *Leçon conv 20/21 re-prouvée : re-auditer les corrections attrape leurs propres bugs.* **3 reports assumés, rattachés CHACUN à sa tâche** (PAS des « dettes » — rien n'est bâclé : ils n'ont pas de sens ici et se font là où ils ont un sens, accrochés pour ressortir au bon moment) : m12 (reaping immédiat d'un orphelin = Job Object natif OU watchdog → **accroché au contenu de T3/T6** ; le pidfile-au-spawn borne déjà la fuite, correctness acquise) · N1 (fsync-répertoire = limite plateforme POSIX/Windows → **notée au contenu de T4**) · N3 (chemin schéma vs packaging asar → **vigilance packaging, plan/00 §2**).
