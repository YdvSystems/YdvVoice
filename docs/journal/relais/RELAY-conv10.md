...
- **🔴 Témoignages** : collecte en cours côté Yohann — la synthèse peut s'intercaler en ouverture de n'importe quelle conv (échéance : avant premier boot Phase 3). **⚠️ Fichiers de portraits/témoignages = PERSONNELS → HORS du dépôt public** (gitignore/dossier privé — `fichier privé` repéré à la racine fin conv 9, à protéger avant tout commit qui l'inclurait).
