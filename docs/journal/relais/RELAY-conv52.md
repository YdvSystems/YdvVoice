...
> **DÉCISION CENTRALE conv 52** : **attaquer V11 — la RÉSIDENCE DES MODÈLES côté voix** (`01-E`, S7 · plan/01 §3 V11). Le set de modèles résident **suit l'état voix** (VEILLE/ÉCOUTE/…) ; le **prewarm au wake est tiré LOCALEMENT** (zéro aller-retour orchestrateur↔sidecar) ; une **allocation VRAM refusée est rapportée sans crash** ; `cmd.model.policy` **émis par l'orchestrateur** intègre les calques du gouverneur socle T7. Test `U-V11` (set résident dérivé de l'état + prewarm local + allocation refusée rapportée) ; co-résidence Whisper+voix, swap RAM→VRAM, coût prewarm → §6 + doc `05`. **Puis V12 (ducking) → V15 ; ensuite `plan/02` (mémoire) / `plan/03` (personnalité).** *(La grille V10 restante — « stop »/« répète »/« moins fort » parlés + DICTÉE/APPROBATION — est repoussée après V11 ; décision Yohann : on avance sur la colonne technique.)*
>
> **conv 51 (PAUSE/REPRISE, V10-partiel) livrée + auditée 4 agents + committée `[conv-51]` (clôture faite).** **Peaufinage ANNEXE entre conv 51 et 52 (session hors-numéro) committé `[peaufinage]`** : le **« hmm » de réflexion** (clip vendorisé joué à **2 s** pour combler le petit blanc quand le cerveau tarde) + le **masqueur « Donne-moi une petite minute »** reculé à **4 s** (« s'il te plaît » retiré) + les **outils du juge** (stats GPU/CPU, capture des respawns + pause/reprise, nettoyage convergent). **Détail + réglages + l'insight « le hmm ne réduit pas la phrase, il retarde un peu les mots » → mémoire [[conv52-hmm-clip]].** **Réglages VERROUILLÉS par Yohann : `hmmAfterMs=2000`, `fillerAfterMs=4000`.** **Ouvert (sans impact perf, GPU idle) : le juge affiche encore `sidecars=4` — PROUVÉ que ce n'est PAS un respawn (`respawns: []`) → fantômes que le nettoyage rate → à creuser [[conv52-juge-readiness-fix]] / [[conv51-juge-fantomes-must-fix]].** **La vraie latence du soir = l'API Anthropic chargée + le WarmBrain qui cale = `plan/02` (pas les fillers, pas le hmm — le hmm ne ralentit PAS le cerveau, prouvé par le TTFT mesuré en amont).**
