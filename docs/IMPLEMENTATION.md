...
> - **Extension couche 2 — base de connaissances (RAG)** *(acté post-clôture conv 5)* : donner des données à Sophia via un **dossier dédié** (ex. `knowledge/`) → **ingestion unique** (découpe en morceaux + embedding BGE-M3) → **consultation par recherche hybride** (FTS5 + `sqlite-vec` + RRF, A10) injectée en contexte = **RAG sur le corpus de Yohann**. **Deux étages distincts** : (a) mémoire **relationnelle** (vous deux, **consolidée la nuit** A12/A18) ; (b) **base de connaissances** (Yohann, **stable, ingérée, jamais réécrite**) — elle lit dans les deux, les traite différemment. **Échelle** : stockage **Go** OK (texte minuscule) ; `sqlite-vec` brute-force **rapide jusqu'à ~dizaines de Mo / dizaines de milliers de morceaux** ; **index ANN** au-delà. Ingestion = coût **unique** par document. **Sophia = cœur du système de Yohann** (élévation de rôle au-delà du cahier → à cadrer pleinement Phase 2). Distinction **connaître** (RAG) vs **utiliser** (couche action).
> - **Adresse naturelle à Sophia — ouvertures ET fermetures sans double-commande** *(A32-étendu, conv 7 ; doc `01`)* : « bonne nuit Sophia », « merci Sophia », la rappeler… doivent marcher **d'un coup**, **à froid comme à chaud**, sans re-dire le wake word d'abord (« Dis-moi Sophia, bonne nuit Sophia » = grotesque + re-machinise). Principe : le **nom « Sophia » dans une phrase naturelle** capte l'intention (wake word entraîné à ça + **mapping d'intention** côté front), cohérent « **ne pas multiplier les commandes vocales** » (A32). **Inventaire complet des interactions véritables** (réveil · « attends » · « laisse tomber » · « bonne nuit » · rappel…) = **grille à dresser en doc `01`**, gardée **minimale**. Fiabilité FR du déclenchement sur « Sophia » dans des phrases variées = **preuve essai à blanc Phase 3** (A8). *(Yohann y tient : « ne pas mettre sous le tapis ».)*
...
- **Phase 2 — Docs techniques** : par couche de dépendance (fichiers séparés, **gabarit 7 rubriques** : arbitrages · interfaces · données · séquences · invariants · acceptation · calibration Phase 3) + plan d'orchestration global (`99`). **Ordre** : `00` socle → `01` vocal → `02` mémoire → `03` personnalité → `04` proactif/tablée → `05` ressources/résilience/coût → `99`. *(**EN COURS — ouverte conv 7.** ✅ **`docs/technique/00-socle-process.md` gravé** : socle process — bi-runtime + gouverneur + boot/durabilité, audit F1–F4 intégré. **Prochain : `01-pipeline-vocal.md`**.)*
