...
- **✅ INCIDENT DONNÉES PERSONNELLES (conv 12) — expurgation EXÉCUTÉE ET PROUVÉE le 2026-07-06** (conversation annexe dédiée — commits `87c9518` + `21733ae`) : docs publics expurgés (pieds « Expurgé le 2026-07-06 »), prénoms→fictifs, historique réécrit + force-push, **preuve par clone frais + grep de tout l'historique**. **Garde pre-commit par contenu ACTIVE** (`.githooks/pre-commit` + blocklist privée). Annexe privée `docs/prive/` (gitignorée) : prompt d'opération + **marbre intégral** (`marbre-sophia.md` — fait foi pour l'installation) + blocklist. *(Retouche factuelle mi-conv 13 — AT1, validée « a ».)*
...
- **Push : régime normal rétabli** (AT1, conv 13) — l'historique réécrit est poussé, `main` = `origin/main` ; commits `[conv-13]` poussés après validation R5.
...
- **✅ FAIT (2026-07-06) — l'opération d'expurgation a été exécutée et prouvée en conversation annexe dédiée** (clone frais + grep de tout l'historique) ; la conv 13 lit les textes post-expurgation. *(Section réécrite en entier à la clôture conv 13 — retouche factuelle AT1.)*
...
### Vigilances
- **Expurgation faite et prouvée (2026-07-06) — push normal rétabli.** Reste, à chaque commit : vérifier qu'aucun `portrait*`/`temoignage*`/`docs/prive/` n'entre, et qu'aucun contenu personnel ne s'écrit dans les fichiers trackés (garde pre-commit par contenu active).
...
- **« Pas de V2 »** · `--bare` jamais (A1) · repo public (gitleaks `pre-commit` + garde contenu **active**, secrets `.env`, identité `Yohann Dandeville <contact@ydvsystems.com>`, **pas de Co-Authored-By**).
