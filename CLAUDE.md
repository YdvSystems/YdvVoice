...
- **🔴 Tâche pré-boot Phase 3 : synthèse des témoignages pré-genèse** — Yohann collecte des portraits d'instants T dans ses conversations Claude passées (prompt complet fourni conv 9, protocole au backlog `IMPLEMENTATION.md`) ; à sa fin de collecte : synthèse par convergence → validation → `user_model` v0. **Échéance : avant le premier boot de la couche mémoire.**
...
- **Phase 3 — Implémentation code** : tâches + tests + critères d'acceptation. **Tâche pré-boot : synthèse des témoignages.**
...
- **🔴 Témoignages pré-genèse** : collecte en cours côté Yohann (moyen terme) — la synthèse peut s'intercaler en ouverture de n'importe quelle conv ; échéance = avant premier boot Phase 3. **⚠️ Les fichiers de témoignages/portraits sont PERSONNELS : à garder HORS du dépôt public** (gitignore ou dossier hors repo — vérifier avant chaque commit ; `fichier privé` repéré à la racine fin conv 9).
